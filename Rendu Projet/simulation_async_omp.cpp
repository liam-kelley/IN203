...
/*
Used to compile: mpic++ -fopenmp -std=c++17 `sdl2-config --cflags` -O3 -march=native -Wall graphisme/src/SDL2/sdl2.o graphisme/src/SDL2/geometry.o graphisme/src/SDL2/window.o graphisme/src/SDL2/font.o graphisme/src/SDL2/event.o graphisme/src/SDL2/texte.o graphisme/src/SDL2/image.o graphisme/src/SDL2/formated_text.o agent_pathogene.o grille.o individu.o simulation_async_omp.cpp -o simulation_async_omp.exe  `sdl2-config --cflags` -lSDL2_ttf -lSDL2_image  `sdl2-config --libs`

Used to execute : export OMP_NUM_THREADS=X; mpiexec -np 2 ./simulation_async_omp.exe
*/
...
void simulation(bool affiche, auto *adressArgc, auto *adressArgv)
{
    ...
    int probeflag = 0;   //Probing purposes
    int Ready = 1; //This could be anything, rank 0 just needs to send something.
    int round1skip = 0;

    MPI_Request request1; //Isend purposes
    MPI_Request request2; //Isend purposes
    ...
    auto& statistiques1 = grille.getStatistiques(); //Doit être constant...
    auto& statistiques0 = grille.getStatistiques(); //Doit être modifiable
    //Bon ça a l'air de marcher au moins

    //Init Chrono
    std::chrono::time_point<std::chrono::system_clock> start, end;
    //Init Moyenne pour q2.3
    double average = 0;
    ...
    {   
        ...
        if (rank >= 1)
        {
            ...
            while(probeflag==false)
            {
                MPI_Iprobe(0,tag,globComm,&probeflag,&status);
            }
            //std::cout << "Rank 1 has seen 0's message!!" << std::endl;
            MPI_Irecv(&Ready, 1, MPI_INT, 0, tag, globComm, &request1);
            //std::cout << "Rank 1 has 'received' the message" << std::endl;
            auto const statistiques1 = grille.getStatistiques(); //on ne veut PAS que ce soit modifié quand ce sera changé a la prochaine itération de la boucle while
            MPI_Isend(statistiques1.data(), largeur_grille*hauteur_grille, stat_struct, 0, tag, globComm, &request2);
            //std::cout << "Rank 1 has sent statistics" << std::endl;
            
        }

        if (rank == 0){

            //Broadcasting to all that you are awaiting a message!
            MPI_Isend(&Ready, 1, MPI_INT, 1, tag, globComm, &request1); //will be a broadcast later on.
            //std::cout << "Rank 0 has sent ready! Awaiting statistics" << std::endl;
            MPI_Recv(statistiques0.data(), largeur_grille*hauteur_grille, stat_struct, 1, tag, globComm, &status);
            //std::cout << "Rank 0 has received statistics. Hooray!! " << std::endl;

            //AFFICHAGE/ECRITURE DES RESULTATS POUR LA JOURNEE ACTUELLE
            if (affiche) afficheSimulation(écran, grille, jours_écoulés, statistiques0.data()); //J'ai du changer affiche simulation
